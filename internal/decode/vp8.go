...
func vp8Keyframe(sample []byte) (image.Image, error) {
	decoder := vp8.NewDecoder()
	decoder.Init(bytes.NewReader(sample), len(sample))

	header, err := decoder.DecodeFrameHeader()
	if err != nil {
		return nil, fmt.Errorf("%w: vp8: %w", ErrCorruptSample, err)
	}
	if !header.KeyFrame {
		return nil, fmt.Errorf("%w: the frame is not a keyframe, so it has no picture of its own", ErrCorruptSample)
	}
	if header.Width <= 0 || header.Height <= 0 {
		return nil, fmt.Errorf("%w: the frame header declares a %dx%d picture",
			ErrCorruptSample, header.Width, header.Height)
	}
	if area := int64(header.Width) * int64(header.Height); area > maxLumaSamples {
		return nil, fmt.Errorf("%w: %dx%d is %d luma samples, over the %d sample cap",
	...
	if picture == nil {
		return nil, fmt.Errorf("%w: the decoder read the frame and produced no picture", ErrCorruptSample)
	}
	return picture, nil
}
