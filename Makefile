...
# The H.264 and HEVC decoders sit behind the h264 and hevc build tags, so there
# are two builds of this library rather than one: the default, with neither, and
# -tags h264,hevc, with both. Every target that compiles or checks the code runs
# both: a break in either configuration is a break.
...
build/cross: ## Build for the supported cross-compilation targets with cgo off
	...
	GOOS=linux GOARCH=arm64 CGO_ENABLED=0 $(GO) build -tags h264,hevc ./...
	GOOS=darwin GOARCH=arm64 CGO_ENABLED=0 $(GO) build -tags h264,hevc ./...
...
test: ## Run tests
	$(GO) test ./... \
		...
		-covermode=atomic
	# The H.264 and HEVC decoders are only in the second run, so that is the one
	# whose coverage profile is kept.
	$(GO) test -tags h264,hevc ./... \
		-coverprofile=coverage.out \
		-covermode=atomic
...
test/perf: ## Compare speed and memory against ffmpeg (needs ffmpeg)
	...
	PERF_OUT="$(abspath $(PERF_OUT))" $(GO) test -tags perf,h264,hevc -run TestCompare -count=1 -timeout 30m -v ./internal/perf/
...
check/go: ## Check Go code quality
	...
	$(GOLINT) run --fix --build-tags h264,hevc $(GOLINT_ARGS) ./...
